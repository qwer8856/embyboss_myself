...
from bot import bot, LOGGER, _open, emby_line, sakura_b, ranks, group, config, bot_name, schedall, webapp
...
async def call_exchange(_, call):
    if not (webapp.status and webapp.url):
        await callAnswer(call, '当前未启用可视化面板，暂时无法使用注册码/续期码。', True)
        return
    await asyncio.gather(
        callAnswer(call, '请前往可视化面板兑换注册码/续期码', True),
        sendMessage(
            call,
            '注册码和续期码已统一改为在可视化面板中使用。请点击下方按钮进入兑换中心，并在网页里完成 CF 验证后再兑换。',
    ...
    if all([e.lv == 'c', e.iv >= _open.exchange_cost, schedall.low_activity]):
        ...
        if m is False:
            ...
            if not changed:
                await asyncio.gather(
                    m.delete(),
                    sendMessage(call, '解封失败：Emby 端未成功恢复，请联系管理员处理。', timer=20),
                )
                return
