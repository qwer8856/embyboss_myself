...
class Embyservice(metaclass=Singleton):
    ...
    @cache.memoize(ttl=20)
    async def get_current_playing_count(self) -> int:
        """
        获取当前播放用户数量
        :return: 播放用户数量
        """
        def _parse_emby_time(raw_value):
            if not raw_value:
                return None
            try:
                ...
                if value.endswith("Z"):
                    value = value[:-1] + "+00:00"
                # 兼容 7 位及以上小数秒（fromisoformat 只支持 6 位）
                if "." in value:
                    if "+" in value[10:] or "-" in value[10:]:
                        sign_index = max(value.rfind("+"), value.rfind("-"))
                        base = value[:sign_index]
                        tz = value[sign_index:]
                    else:
                        base = value
                        tz = ""
        ...
        try:
            ...
            if result.success and result.data:
                ...
                for session in result.data:
                    ...
                    heartbeat = session.get("LastPlaybackCheckIn") or session.get("LastActivityDate")
                    hb_time = _parse_emby_time(heartbeat)
                    if hb_time is not None:
                        if hb_time.tzinfo is None:
                            hb_time = hb_time.replace(tzinfo=timezone.utc)
                        # 超过 3 分钟未心跳，视为非在线播放
                        if (now - hb_time.astimezone(timezone.utc)).total_seconds() > 180:
                            continue

                    count += 1
                LOGGER.debug(f"当前在线播放数: {count} (会话总数: {len(result.data)})")
                return count
            else:
    ...
    async def get_medias_count() -> str:
        ...
        :return: 统计文本
        """
        base_url = str(emby_url or "").rstrip("/")
        if not base_url:
            """
            LOGGER.error("获取媒体统计异常: emby_url 未配置")
            return "🤕Emby 服务器连接失败!"
            """
            LOGGER.error("get_medias_count failed: emby_url not configured")
            return "🤕Emby 服务器连接失败!"
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, params={"api_key": emby_api}, headers={"accept": "application/json"}) as response:
                    if response.status == 200:
                        try:
                            ...
                            LOGGER.error(
                                f"获取媒体统计解析异常: {type(decode_err).__name__}: {decode_err}, "
                                f"url={url}, body={body[:200]!r}"
                            )
                        ...
                        LOGGER.error(f"获取媒体统计失败: HTTP 204, url={url}")
                        return "🤕Emby 服务器返回数据为空!"
                    else:
                        body = await response.text()
                        LOGGER.error(f"获取媒体统计失败: HTTP {response.status}, url={url}, body={body[:200]!r}")
                        return "🤕Emby 服务器返回数据为空!"
        except asyncio.TimeoutError:
            LOGGER.error(f"获取媒体统计异常: 请求超时, url={url}")
            return "🤕Emby 服务器连接失败!"
        except aiohttp.ClientError as e:
            LOGGER.error(f"获取媒体统计异常: {type(e).__name__}: {e}, url={url}")
            return "🤕Emby 服务器连接失败!"
        except Exception:
            LOGGER.exception(f"获取媒体统计异常: url={url}")
            return "🤕Emby 服务器连接失败!"
