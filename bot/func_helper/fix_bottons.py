...
from bot import chanel, main_group, bot_name, extra_emby_libs, tz_id, tz_ad, tz_api, tz_version, tz_username, tz_password, _open, sakura_b, \
    schedall, auto_update, fuxx_pitao, moviepilot, red_envelope, config, LOGGER, webapp
...
def webapp_url_with_panel_view(view_id: str) -> str:
    """
    为 WebApp 按钮带上 panel_view 查询参数，便于前端直达某视图。
    不用 #hash：部分 Telegram 客户端打开 Mini App 时会丢失 fragment，只会落在主页。
    """
    base = (webapp.url or "").strip()
    if not base or not view_id:
        return base
    if "#" in base:
        base = base.split("#", 1)[0]
    q = quote(view_id, safe="")
    sep = "&" if ("?" in base) else "?"
    return f"{base}{sep}panel_view={q}"


def webapp_panel_ikb(text: str, view_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                text=text,
                web_app=WebAppInfo(url=webapp_url_with_panel_view(view_id)),
            ),
        ]
    ])
...
def judge_start_ikb(is_admin: bool, account: bool) -> InlineKeyboardMarkup:
    ...
    start面板按钮
    """
    web_panel_ok = bool(webapp.status and webapp.url)
    if not account:
        d = []
        if not web_panel_ok:
        ...
        if schedall.check_ex and not web_panel_ok:
    ...
    if web_panel_ok and show_redeem_button:
        ...
        keyword.inline_keyboard.append([
            InlineKeyboardButton(
                text=redeem_text,
                web_app=WebAppInfo(url=webapp_url_with_panel_view('redeem-center')),
            ),
        ])
    # 签到仅在 WebApp 内完成（Turnstile 等）；未启用面板时不展示签到以免绕过验证
    if web_panel_ok and _open.checkin:
        keyword.inline_keyboard.append([
            InlineKeyboardButton(
                text='🎯 签到',
                web_app=WebAppInfo(url=webapp_url_with_panel_view('checkin-points')),
            ),
        ])
    return keyword
